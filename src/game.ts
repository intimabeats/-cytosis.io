...
import { AIController } from './ai'; // Import AIController
import {
  GameState,
  Player,
  PowerUpType,
...
export class Game {
  ...
  aiControllers: Map<string, AIController>; // Use AIController
  ...
  createSafeZone(position: Vector2D, radius: number): void {
    ...
    this.gameState.players.forEach((player, _id) => { // Usando _id para unused
      if (player.isAI) {
